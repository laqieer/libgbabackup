...
#include "gba_flash_512k.h"

extern u8 AgbLibFlash512KVersion[];
...
//---------------------------------------------------------------------------------
    ...
    iprintf("GBA FLASH 512K Test\nhttps://github.com/laqieer/libgbabackup\n\n");

    if (IdentifyFlash()) {
        iprintf("Error: unsupported device!\nTest failed!");
    } else {
        InitFlashTimer();
        u32 err = ProgramFlashSectorAndVerifyNBytes(0, AgbLibFlash512KVersion, 20);
        u8 s[20];
        switch (err) {
            case 0:
                ReadFlash(0, 0, s, 20);
